...
#                     print("Progress {:.1f} %".format(100*num/total_num))
    ...
    data_csv = pd.read_csv("Combined_dataset.csv",iterator=True)
    data_df = _read_and_form_combined_dataframe(data_csv)
    Combined = Bunch()
    Combined.data = _get_Combineddata(data_df)
    Combined.target = _get_Combinedtarget(data_df)
    Combined.DESCR = _get_Combineddescr(data_df)
    Combined.feature_names = _get_feature_names(data_df)
    Combined.target_names = _get_target_names()
    return Combined